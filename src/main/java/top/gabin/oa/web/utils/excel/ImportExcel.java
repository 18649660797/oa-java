package top.gabin.oa.web.utils.excel;
...
import top.gabin.oa.web.utils.Reflections;
...
/**
 * 导入Excel文件（支持“XLS”和“XLSX”格式） 工具类
 */
public class ImportExcel {
	 ...
	 * @param headerNum 标题行号， 如果没有标题,则 headerNum请设置为0
	...
	/**
	 *  获取数据起始行的 下标(excel 里的行号从1开始,对应数组下标的 0开始)
     *
     *  数据行在 headerNum 的下一行
     *   1. 如果 headerNum => 1，则 数据行的下标为 headerNum -1 +1 --> :即 headerNum-1(标题行的下标) +1(数据行号=标题行号+1)
     *   2. 如果 标题不存在,则 数量行从 第一行开始,即 下标为 0
     *
	 * @return
	 */
		...
		for (int i = this.getDataRowNum(); i <= this.getLastDataRowNum(); i++) {
			...
			for (Object[] os : annotationList){
				...
				if (val != null){
					...
					//log.info("Import value type: ["+i+","+column+"] " + valType);
					try {
