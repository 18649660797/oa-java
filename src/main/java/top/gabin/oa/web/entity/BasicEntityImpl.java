...
@FilterDef(defaultCondition =":" + TenantUtils.FILTER_TENANT_ID + " = TENANT_ID", name = TenantUtils.FILTER_TENANT, parameters = {@ParamDef(name = TenantUtils.FILTER_TENANT_ID, type = "long")})
@Filters({ @Filter(name = TenantUtils.FILTER_TENANT) })
// 将所有注解映射到子类去,父类不会产生新表
@MappedSuperclass
public class BasicEntityImpl implements BasicEntity {
